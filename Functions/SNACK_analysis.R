...
# only trans 1->2 #
###################
model_age_cov2 <- msm(MP ~ Age, subject=lopnr, data=snack_2_filtered, qmatrix= q0, deathexact = 3,
                     covariates = list("1-2" = ~ Age + educ_el + dm_sex + ALCO_CONSUMP+ R_SMOKE + no_pa + life_alone, "1-3" = ~ Age, "2-3" = ~ Age), 
                     method = "BFGS", control = list(fnscale = 14000, maxit= 10000))
model_age_cov2
model_age_cov3 <- msm(MP ~ Age, subject=lopnr, data=snack_2_filtered, qmatrix= q0, deathexact = 3,
                      covariates = list("1-2" = ~ Age + dm_sex + no_pa + life_alone + fin_strain_early+finstrain+sei_long_cat, "1-3" = ~ Age, "2-3" = ~ Age), 
                      method = "BFGS", control = list(fnscale = 14000, maxit= 10000))
model_age_cov3

model_age_cov4 <- msm(MP ~ Age, subject=lopnr, data=snack_2_filtered, qmatrix= q0, deathexact = 3,
                      covariates = list("1-2" = ~ Age + educ_el + dm_sex + no_pa + life_alone + ALCO_CONSUMP+ R_SMOKE+ fin_strain_early+finstrain+sei_long_cat, "1-3" = ~ Age, "2-3" = ~ Age), 
                      method = "BFGS", control = list(fnscale = 14000, maxit= 10000))
hazard.msm(model_age_cov4)
...
model_5.1_misc <- msm(MP ~ Age, subject=lopnr, data=snack_nhm, qmatrix= qmatrix_init, 
                      ...
                      #fixedpars= c(6:n2),
                      control = list(fnscale = 20000, maxit= 10000))
# does not work -> maybe should filter out NAs
...
# influential covariates: dm_sex, no_pa, life_alone
model_final <- msm(MP ~ Age, subject=lopnr, data=snack_2_filtered, qmatrix= q0, deathexact = 3,
                      covariates = list("1-2" = ~ Age + dm_sex + no_pa + life_alone, "1-3" = ~ Age, "2-3" = ~ Age), 
                      method = "BFGS", control = list(fnscale = 14000, maxit= 10000))
model_final
qmatrix.msm(model_final)
sojourn.msm(model_final)
hazard.msm(model_final)

qmatrix_init <- model_final$Qmatrices$baseline 
covinits_init <-model_final$estimates
n1<- sum(q0 > 0)
n2 <- length(model_final$estimates)
covinits_init <-as.list(covinits_init[(n1+1):n2])
#n1<- sum(q0 > 0) + sum(covinits_init!=0)
covinits_init <- covinits_init[covinits_init != 0]
n1 <- sum(q0 > 0) +length(covinits_init)
...
model_fin_misc <- msm(MP ~ Age, subject=lopnr, data=snack_2_filtered, qmatrix= qmatrix_init, 
                      covinits = covinits_init, # passare lista valori age a convergenza
                      deathexact = 3,
                      initprobs = c(0.8,0.2,0),
                      covariates = list("1-2" = ~ Age + dm_sex + no_pa + life_alone,"1-3" = ~ Age, "2-3" = ~ Age),
...
nonh <- q_nhm
#dm_sex + no_pa + life_alone + fin_strain_early+finstrain+sei_long_cat
covm <- list(
